...
class Fishpig_Smushit_Model_Resource_Image extends Fishpig_Smushit_Model_Resource_Image_Hack
{
	...
	public function createUsingResult($file, array $result)
	{
		$data = array(
			...
			'src_size' => isset($result['src_size']) && !empty($result['src_size']) ? $result['src_size'] : 0,
			'dest_size' => isset($result['dest_size']) && !empty($result['dest_size']) ? $result['dest_size'] : 0,
			'percent' => isset($result['percent']) && !empty($result['percent']) ? $result['percent'] : 0,
			'created_at' => now(),
		);
