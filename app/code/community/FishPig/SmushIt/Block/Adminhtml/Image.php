...
class Fishpig_SmushIt_Block_Adminhtml_Image extends Mage_Adminhtml_Block_Widget_Tabs
{
	...
	{
		try {
			Mage::helper('smushit')->validateCronRunning();
		}
		catch (Exception $e) {
			$html .= sprintf(
				"<script type=\"text/javascript\">$('page:main-container').select('.content-header').first().insert({after: (new Element('ul', {class: 'messages'})).update((new Element('li', {class: 'notice-msg'})).update('%s'))});</script>",
				$e->getMessage()
			);
		}
